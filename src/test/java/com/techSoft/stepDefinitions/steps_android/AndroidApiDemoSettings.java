...
public class AndroidApiDemoSettings {
    ...
    public AndroidApiDemoSettings() {
        if (BaseTest.getAndroidDriverInstance() == null) {
            try {
                new BaseTest().getAndroidDriver();
            } catch (Exception e) {
                throw new RuntimeException("Error : Failed to Initialize AndroidDriver", e);
            }
        }
        this.driver = BaseTest.getAndroidDriverInstance();
        Assert.assertNotNull(driver, "AndroidDriver is Not Initialized!");
    }
